...
namespace WZK
{
    ...
    public class SoundManager : MonoBehaviour
    {
        ...
        {
            if (SwitchSoundID(type) != SoundID.背景音乐) Debug.LogError("播放类型不对，应用MusiceType！");
        ...
        public Sound PlayFromResource(Enum type, bool isRepeatSame = true)
        {
            ...
            return PlaySound(clip, soundName, isRepeatSame).SetID(SwitchSoundID(type).ToString());
        }
        /// <summary>
        ...
        {
            List<Sound> destoryList = SearchSound(soundName);
            if (isComplete)
            {
        ...
        public void DestroySound(string soundName,bool isDeletOnce,bool isComplete)
        {
            List<Sound> destoryList = SearchSound(soundName);
            if (isComplete)
            {
                destoryList[0].Finish();
              //  destoryList.ToList().ForEach(x => x.Finish());
            }
            else
            {
                destoryList[0].FinishNoComplete();
                //destoryList.ToList().ForEach(x => x.FinishNoComplete());
            }
        }
